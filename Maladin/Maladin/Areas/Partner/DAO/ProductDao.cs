...
namespace Maladin.Areas.Partner.DAO
{
    public class ProductDao
    {
        ...
        {
            var count = (from m in dbContext.PRODUCTs
                        where m.ID_PRODUCT.Contains(id)
                        select m).Count();
        ...
        public List<PRODUCT> GetListProductByPage(int pageSize, int page, string q)
        {
            ...
            if (size >= 0)
            {
                ...
                " RowConstrainedResult.NOTE_PRODUCT,RowConstrainedResult.ID_INFO, RowConstrainedResult.DATE_PRODUCT, RowConstrainedResult.USER_ACC, RowConstrainedResult.IS_SELL " +
                " FROM ( SELECT ROW_NUMBER() OVER ( ORDER BY DATE_PRODUCT ) AS RowNum, *FROM dbo.PRODUCT ) AS RowConstrainedResult" +
                " WHERE ID_PRODUCT LIKE '%"+q + "%' and RowNum > " + Convert.ToString(pageSize * (page - 1)) + " AND RowNum <= " + Convert.ToString(page * pageSize) + " ORDER BY RowNum";
                    var data = dbContext.Database.SqlQuery<PRODUCT>(sql)
                        .Select(b => new PRODUCT
                        {
                            ID_PRODUCER = b.ID_PRODUCER,
                ...
                " RowConstrainedResult.NOTE_PRODUCT,RowConstrainedResult.ID_INFO, RowConstrainedResult.DATE_PRODUCT, RowConstrainedResult.USER_ACC, RowConstrainedResult.IS_SELL " +
                " FROM ( SELECT ROW_NUMBER() OVER ( ORDER BY DATE_PRODUCT ) AS RowNum, *FROM dbo.PRODUCT ) AS RowConstrainedResult" +
                " WHERE ID_PRODUCT LIKE '%" + q + "%' and RowNum > " + Convert.ToString(pageSize * (page - 1)) + " AND RowNum <= " + Convert.ToString(count) + " ORDER BY RowNum";
                    var data = dbContext.Database.SqlQuery<PRODUCT>(sql)
                        .Select(b => new PRODUCT
                        {
                            ID_PRODUCER = b.ID_PRODUCER,
